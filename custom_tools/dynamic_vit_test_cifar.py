...
t = torchvision.transforms.Normalize(cfg.data_preprocessor["mean"], cfg.data_preprocessor["std"])
# transform = torchvision.transforms.Compose([
#     torchvision.transforms.Normalize(cfg.data_preprocessor["mean"], cfg.data_preprocessor["std"]),
#     # torchvision.transforms.ToTensor()
# ])
def collate_fn(batchs):
    tensors, targets = [], []
    for batch in batchs:
        item_img = batch["inputs"]
        item_label = batch["data_samples"].gt_label
        tensors.append(item_img)
        targets.append(item_label)
    # F.normalize()
    tensors = torch.stack(tensors).float()
