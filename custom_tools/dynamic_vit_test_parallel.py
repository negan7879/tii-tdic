...
write_dict["acc_ic6"] = []
write_dict["acc_ic9"] = []
test_loader = tqdm.tqdm(test_loader, file=sys.stdout)
with torch.no_grad():
    # 预热model
    ...
    for index, (threshold_, prob_) in enumerate(zip(output, probs_list)):
        ...
        for step, data in enumerate(test_loader):
            ...
            # torch.cuda.synchronize()
            # duration_ = starter.elapsed_time(ender)
        ...
        total_sec = np.sum(duration)
        print("mean time = ", total_sec)
        test_loader = tqdm.tqdm(test_loader, file=sys.stdout)
        # total_time = 0.0
        # for ic in ic_location:
